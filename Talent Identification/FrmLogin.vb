...
Public Class FrmLogin
    ...
    Private Sub Login()
        Try
            ...
            _cmd =
                New MySqlCommand(
                    "Select * from users where BINARY username = '" + txtusername.Text + "' and Password = '" + txtpwd.Text + "'",
                    _con)
    ...
    Private Sub Label7_TextChanged(sender As Object, e As EventArgs) Handles Label7.TextChanged
        If Label7.Text = "58" Then
            ...
            Dim myForm As New TalentID() 'go to the menu after logging in
    ...
    Private Sub txtusername_TextChanged(sender As Object, e As EventArgs) Handles txtusername.TextChanged
        Try
            _con.Close()
            _con.Open()
            _cmd = New MySqlCommand("select usergroup,industry from users where username='" & txtusername.Text & "'  ", _con)
            Dim dr As MySqlDataReader = _cmd.ExecuteReader()
            While dr.Read()
                Dim str As String = dr.GetString("usergroup").ToString()
                Dim str2 As String = dr.GetString("industry").ToString()
